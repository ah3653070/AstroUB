...
from astro.plugins.sql.locks_sql import get_locks, is_locked, update_lock
from astro.utils import admin_cmd


@astro.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@astro.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@astro.on(admin_cmd(pattern="unlock ?(.*)"))
@astro.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@astro.on(admin_cmd(pattern="currenabledlocks"))
@astro.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@astro.on(events.MessageEdited())  # pylint:disable=E0602
@astro.on(events.NewMessage())  # pylint:disable=E0602
...
@astro.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
