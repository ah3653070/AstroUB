...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await astro.download_media(
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1258905497:
            await event.edit("Hey You Ramdi Mother Fucker🙂He is my Owner")
            return
    except BaseException:
        if FBAN == "@Alone_loverboy":
            await event.edit("Hey You Ramdi MotherFucker🙂He is my owner")
            return
    ...
    if not len(fedList):
        for a in range(3):
            async with astro.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await astro.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.EXCLUDE_FED and fed in excludeFed:
            await astro.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await astro.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await astro.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
...
async def _(event):
    ...
    for fed in fedList:
        await astro.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(5)
        await astro.send_message(chat, f"/unfban {FBAN}")
        await asyncio.sleep(5)
